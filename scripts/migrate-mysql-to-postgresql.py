...
psql_conn = psycopg2.connect("dbname=pajbot options='-c search_path=pajbot1_bulldog'")
print("done.")
...
with mysql_conn.cursor() as mysql, psql_conn.cursor() as psql:
    ...
    copy_table("dotabet_game", ["id", "internal_id", "outcome", "points_change", "win_betters", "loss_betters"])
    copy_auto_increment("dotabet_game", "id")

    copy_table("dotabet_bet", ["id", "game_time", "user_id", "outcome", "points", "profit"])
    copy_auto_increment("dotabet_bet", "id")
    ...
    copy_table("longtimeout", ["username", "timeout_start", "timeout_recent_end", "timeout_end", "timeout_author"])

    copy_table("module", ["id", "enabled", "settings"], {"enabled": coerce_to_boolean})

    copy_table("playsound", ["name", "link", "volume", "cost", "cooldown", "enabled"], {"enabled": coerce_to_boolean})
