...
def init(page):
    ...
    def autoresponses(**options):
        with DBManager.create_session_scope() as db_session:
            autoresponses = (
                db_session.query(Autoresponse)
                .options(
                    joinedload(Autoresponse.data).joinedload(AutoresponseData.user),
                    joinedload(Autoresponse.data).joinedload(AutoresponseData.user2),
                )
                .all()
            )
            return render_template("admin/autoresponses.html", autoresponses=autoresponses)
        ...
        if request.method == "POST":
            ...
            with DBManager.create_session_scope(expire_on_commit=False) as db_session:
                if id is not None:
                    autoresponse = (
                        db_session.query(Autoresponse)
                        .options(joinedload(Autoresponse.data))
                        .filter_by(id=id)
                        .one_or_none()
                    )
    ...
    def autoresponses_edit(autoresponse_id, **options):
        with DBManager.create_session_scope() as db_session:
            ...
            return render_template("admin/create_autoresponse.html", autoresponse=autoresponse)
