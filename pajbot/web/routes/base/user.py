...
def init(app):
    ...
    def user_profile_connections(login):
        with DBManager.create_session_scope() as db_session:
            ...
            discord = None
            steam = None
            if 'discord_id' in session and 'discord_username' in session and session['discord_id'] is not None and session['discord_username'] is not None:
            ...
            try:
                saved_data = db_session.query(UserConnections).filter_by(twitch_id=session['user']['id']).one().jsonify()
            except:
                saved_data = None
            if saved_data:
                return render_template("connections_unlink.html", user=user, data=saved_data, twitch_user=session['user'], returnUrl=f'/user/{login}/connections')
            return render_template("connections.html", user=user, data=data, returnUrl=f'/user/{login}/connections', pair_failed=False)
            ...
            if user.offcd:
                ...
                try:
                    if discord is not None and steam is not None: 
                        UserConnections._create(db_session, session['user']['id'], session['discord_id'], session['discord_username'], session['steam_id'])
                        user._setcd(db_session)
                        db_session.commit()
                        return redirect(f"/user/{login}/connections/")
                    else:
                        return render_template("connections.html", user=user, data=data, returnUrl=f'/user/{login}/connections', pair_failed=True)
