def up(cursor, bot):
    ...
    cursor.execute(
        """
    CREATE TYPE bet_outcome AS ENUM('win', 'loss');
    """
    )
    ...
    CREATE TABLE bet_game (
        ...
        win_bettors INT NOT NULL,
        loss_bettors INT NOT NULL
    )
    """
    )
    ...
    CREATE TABLE bet_bet (
        id INT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
        game_id INT NOT NULL REFERENCES bet_game(id),
    ...
    cursor.execute("CREATE INDEX ON bet_bet(game_id)")
    cursor.execute("CREATE INDEX ON bet_bet(user_id)")
