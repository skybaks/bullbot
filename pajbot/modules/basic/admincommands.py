        ...
        self.commands["editpoints"] = Command.raw_command(
            ...
            examples=[
                CommandExample(
                    None,
                    "Give a user points",
                    chat="user:!editpoints synderen 500\n" "bot>user:Successfully gave syndereN 500 points.",
                    description="This creates 500 points and gives them to syndereN",
                ).parse(),
                CommandExample(
                    None,
                    "Remove points from a user",
                    chat="user:!editpoints synderen -500\n" "bot>user:Successfully removed 500 points from syndereN.",
                    description="This removes 500 points from syndereN. Users can go into negative points with this.",
                ).parse(),
            ],
        )
        self.commands["setpoints"] = Command.raw_command(
            ...
            examples=[
                CommandExample(
                    None,
                    "Set a user's points",
                    chat="user:!setpoints synderen 500\n" "bot>user:Successfully set syndereN's points to 500.",
                    description="This sets syndereN's points to 500.",
                ).parse()
            ],
        )
        self.commands["level"] = Command.raw_command(self.level, level=1000, description="Set a users level")

        self.commands["silence"] = Command.raw_command(self.cmd_silence, level=1500, description="Silence the bot")
        self.commands["mute"] = self.commands["silence"]

        self.commands["unsilence"] = Command.raw_command(self.cmd_unsilence, level=1500, description="Unsilence the bot")
