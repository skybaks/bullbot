...
class NukeModule(BaseModule):
    ...
    def nuke_command(self, **options):
        ...
        if options["whisper"]:
            bot.say("Do it in chat DansGame")
            return False
    ...
    def actually_nuke(self, message, bot, source):
        if message and len(message) > 0:
            ...
            url = "https://overrustlelogs.net/Admiralbulldog" " chatlog/{:%B %Y}/{:%Y-%m-%d}.txt".format(today, today)
            log.debug(url)
            ...
            reason = '{} nuked {} users for the phrase "{}" for {}'.format(
                source.username, len(badUsers), phrase, self.format_time(duration)
            )
            i = 1
    ...
    def load_commands(self, **options):
        self.commands["oldnuke"] = pajbot.models.command.Command.raw_command(
            self.nuke_command,
            level=420,
            delay_all=0,
            delay_user=0,
            ...
            examples=[
                pajbot.models.command.CommandExample(
                    ...
                    description="Nuke the last 30 messages that have 'TriHard' in them for 1 minute",
                ).parse(),
                pajbot.models.command.CommandExample(
                    None,
                    ...
                    description="Nuke the last 100 messages that have 'I like boats' in them for 3 minutes",
                ).parse(),
                pajbot.models.command.CommandExample(
                    None,
                    ...
                    description="Nuke the last 100 messages that match the '\bnam' regex for 1 hour",
                ).parse(),
            ],
        )
        self.commands["tcpurge"] = pajbot.models.command.Command.raw_command(
            self.monkeypurge,
            level=500,
            delay_all=0,
            delay_user=0,
