    ...
    def base_paid_timeout(self, bot, source, message, _time, _cost):
        ...
        with DBManager.create_session_scope() as db_session:
            ...
            if victim.timeout_end is not None and victim.timeout_end > now:
                ...
                bot.whisper(victim, f"{source} just timed you out for {_time} seconds.")
                bot.timeout(victim, _time, reason=f"Timed out by {source} for {_cost} points")
                bot.say(f"{source} timed out {victim} with !timeout for {_cost} points!")
