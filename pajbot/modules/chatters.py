        ...
        with RedisManager.pipeline_context() as pipeline:
            with DBManager.create_session_scope() as db_session:
                ...
                for num_points, usernames in points_to_give_out.items():
                    payload = {User.points: User.points + num_points}
                    if self.bot.is_online:
                        payload[User.minutes_in_chat_online] = (
                            User.minutes_in_chat_online + self.update_chatters_interval
                        )
                    else:
                        payload[User.minutes_in_chat_offline] = (
                            User.minutes_in_chat_offline + self.update_chatters_interval
                        )
                    db_session.query(User).filter(User.username.in_(usernames)).update(
    ...
    def enable(self, bot):
        ...
        if bot:
            if not self.initialized:
                ...
                log.error("XXXXXXXXXX THIS SHOULD NOT HAPPEN")
                log.error("XXXXXXXXXX THIS SHOULD NOT HAPPEN")
                log.error("XXXXXXXXXX THIS SHOULD NOT HAPPEN")
                log.error("XXXXXXXXXX THIS SHOULD NOT HAPPEN")
                log.error("XXXXXXXXXX THIS SHOULD NOT HAPPEN")
