...
class AutoresponseModule(BaseModule):
    ...
    def add_autoresponse(self, **options):
        ...
        if message:
            ...
            if new_autoresponse is True:
                bot.whisper(
                    source.username, "Added your autoresponse (ID: {autoresponse.id})".format(autoresponse=autoresponse)
                )
            ...
            bot.whisper(
                source.username,
                "Updated your autoresponse (ID: {autoresponse.id}) with ({what})".format(
                    autoresponse=autoresponse, what=", ".join([key for key in options if key != "added_by"])
                ),
            )
    ...
    def load_commands(self, **options):
        self.commands["add"] = pajbot.models.command.Command.multiaction_command(
            ...
            command="add",
            commands={
                "autoresponse": pajbot.models.command.Command.raw_command(
                    self.add_autoresponse,
                    level=500,
                    ...
                    examples=[
                        pajbot.models.command.CommandExample(
                            ...
                            description="This creates a autoresponse with the default settings. Whenever a user types testman123 in chat the bot will tag them and say 'yikes'",
                        ).parse(),
                        pajbot.models.command.CommandExample(
                            None,
                            "Make it so a response is sent through whisper",
                            chat="user:!add autoresponse testman123 yikes --whisper\n"
                            "bot>user:Updated the given autoresponse (ID: 83) with (whisper)",
                            description="Changes a command so that the response would only be sent through a whisper.",
                        ).parse(),
                    ],
                )
            },
